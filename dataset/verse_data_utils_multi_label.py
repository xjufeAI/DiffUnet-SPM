...
class PretrainDataset(Dataset):
    def __init__(self, num_classes,sample_list,raw_list,msk_list, transform=None, cache=False) -> None:
        super().__init__()
        ...
        if cache:
            self.cache_data = []
            for i in tqdm(range(len(raw_list)), total=len(raw_list)+len(msk_list)):
                d  = self.read_data(raw_list[i],msk_list[i])
                self.cache_data.append(d)

    def read_data(self, raw_path,msk_path):
        image_path =raw_path
        seg_path = msk_path
        ...
        label=sitk.ReadImage(image_path)
        spacing = np.array(image.GetSpacing())
        seg_data = sitk.GetArrayFromImage(label)


        #image_data = np.expand_dims(np.array(image_data).astype(np.float32),axis=0)        
    ...
    def __getitem__(self, i):
        if self.cache:
            ...
            try:
                image = self.read_data(self.raw_list[i],self.msk_list[i])
            except:
                with open("./bugs.txt", "a+") as f:
                    f.write(f"数据读取出现问题，{self.raw_list[i]}\n")
...
def get_loader_verse(data_dir,list_dir,batch_size=1,fold=0, num_workers=8):
    ...
    train_data = data_dir + "/dataset-01training"
    validation_data = data_dir + "/dataset-02validation"
    test_data = data_dir + "/dataset-03test"
    ...
    raw_test_files = glob.glob(os.path.join(test_data+ 'rawdata/*/*nii.gz'))
    ...
    masks_test_files = glob.glob(os.path.join(test_data+ 'derivatives/*/*nii.gz'))
    
    train_size = len(train_list)
    val_size = len(val_list)
    test_size=len(test_list)
    
    train_img_files=[i for i in raw_train_files if  i.split('_')[0].split('/')[-1] in train_list]
    train_msk_files=[i for i in masks_train_files if  i.split('_')[0].split('/')[-1] in train_list]
    
    val_img_files=[i for i in raw_validation_files if  i.split('_')[0].split('/')[-1] in val_list]
    val_msk_files=[i for i in masks_validation_files if  i.split('_')[0].split('/')[-1] in val_list]
    
    test_img_files=[i for i in raw_test_files if  i.split('_')[0].split('/')[-1] in test_list]
    test_msk_files=[i for i in masks_test_files if  i.split('_')[0].split('/')[-1] in test_list]
    
    print(f"train_img is {len(train_img_files)}, train_msk is {len(train_msk_files)},val_img is {len(val_img_files)}, val_msk is {len(val_msk_files)},test_img is {len(test_img_files)}, test_msk is {len(test_msk_files)}")
    
    img_size=[128,160,96]
    train_ds = PretrainDataset(int (26),train_list,train_img_files,train_msk_files,
                               transform=transforms.Compose([RandomGenerator(output_size=img_size, mode = 'train')]))

    val_ds = PretrainDataset(int (26),val_list,val_img_files,val_msk_files,
                                     transform=transforms.Compose(
                                   [RandomGenerator(output_size=img_size, mode = 'val')]))
    

    test_ds = PretrainDataset(int (26),test_list,test_img_files, test_msk_files,
                              transform=transforms.Compose(
