...
class BasicUNetDe(nn.Module):
    ...
    def __init__(
        ...
        self.learnable_shape_prior = nn.Parameter(torch.randn(1, out_channels, config.n_patches))
    ...
    def forward(self, x: torch.Tensor, t, embeddings=None, image=None):
        ...
        x3, refined_shape_prior = self.spm1(x3, learnable_shape_prior)
        u4 = self.upcat_4(x4, x3, temb)
        
        x2, refined_shape_prior = self.spm2(x2, learnable_shape_prior)
        u3 = self.upcat_3(u4, x2, temb)
        
        x1, refined_shape_prior = self.spm3(x1, learnable_shape_prior)
